...
const App = () => {
  ...
  if (loading) {
    conditionalContent = (
      <div className={`w-full h-full bg-white p-12 flex items-center justify-center ${darkMode ? ("bg-stone-800") : ("bg-white")}`}>
        <p className="text-2xl">Loading...</p>
      </div>
    );
  } else if (!loading && !error) {
    conditionalContent = (
      <div className={`w-full h-full bg-white p-10 ${darkMode ? ("bg-stone-800") : ("bg-white")}`}>
